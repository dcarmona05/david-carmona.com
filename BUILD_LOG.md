...
**Update:** `git init` is done and the repo is pushed to GitHub at `https://github.com/dcarmona05/david-carmona.com` (branch `main`, HTTPS remote via a personal access token entered interactively — no token stored in this repo). This is no longer at risk from a sandbox wipe. Keep committing and pushing as work continues.
